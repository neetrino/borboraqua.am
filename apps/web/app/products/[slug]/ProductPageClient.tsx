...
export function ProductPageClient({ initialProduct, slug, variantIdFromUrl }: ProductPageClientProps) {
  ...
  }, [initialProduct, product, variantIdFromUrl]);
  ...
  }, [slug, variantIdFromUrl, router, fetchProduct, initialProduct]);
  ...
  const switchToVariantImage = useCallback((variant: ProductVariant | null) => {
    ...
    for (const url of splitUrls) {
      ...
      if (isAttributeValueImage(processedUrl)) continue;
      ...
      const imageIndex = images.findIndex(img => {
        ...
        if (!processedImg.startsWith('data:') && !processedUrl.startsWith('data:')) {
          ...
          if (imgFilename && processedFilename && imgFilename === processedFilename) return true;
        }
